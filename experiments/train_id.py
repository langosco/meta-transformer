...
from meta_transformer.data import Data, DataLoaderTrainId, load_batches
...
def create_loss_fn(model_forward: callable):
    ...
    def loss_fn(
        ...
        loss = jnp.mean((outputs - data.input)**2)
...
#    parser.add_argument('--num_heads', type=int, help='Number of heads', default=16)
    ...
    subrng, rng = jax.random.split(rng)
    train_loader, val_loader, unnormalize_data = load_data(
...
#        )
    ...
    if args.load_checkpoint is None:
        ...
        state = checkpointer.restore(args.load_checkpoint / "params")


    checkpoint_savedir = epath.Path(output_dir) / "mm-checkpoints" \
        / "train_id" / args.poison_type / f"run_{int(time())}"
    ...
    def get_reconstruction_metrics(meta_model_outputs):
        base_params = unnormalize_data(meta_model_outputs)
    ...
    wandb.init(
        ...
        project=f"train-id-{args.dataset}",
        tags=args.tags,
        notes=args.notes,
        config={
    ...
    def eval_accuracy_of_base_models():
        logger.info("Base models (training set):")
        for batch in train_loader:
            acc = get_reconstruction_metrics(batch.input)['out/accuracy']
            logger.info(f"Base model test accuracy: {acc}")
        

        print()
        print()
        logger.info("Base models (validation set):")
        for batch in val_loader:
            acc = get_reconstruction_metrics(batch.input)['out/accuracy']
            logger.info(f"Base model test accuracy: {acc}")
    ...
    if args.save_checkpoint:
        ...
        checkpointer.save(checkpoint_savedir / "checkpoint", state.params)
